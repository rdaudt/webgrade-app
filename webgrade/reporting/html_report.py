...
def _plain_language_annotation(annotation: dict[str, Any], report_context: dict[str, Any]) -> tuple[str, str]:
    title = " ".join(str(annotation.get("title") or "").split())
    caption = " ".join(str(annotation.get("caption") or "").split())
    audience = ", ".join(str(item) for item in report_context.get("report_audience", [])[:3])
    audience_phrase = audience or "decision-makers"

    replacements = {
        "hero": "opening section",
        "cta": "next step",
        "underpowered": "not doing enough to guide visitors",
        "shallow": "too limited",
        "density": "crowding",
        "cluttered": "busy",
    }
    ...
    def _rewrite(text: str) -> str:
        ...
        for source, target in replacements.items():
            ...
            rewritten = rewritten.replace(source.title(), target.capitalize())
            rewritten = rewritten.replace(source.upper(), target.upper())
        return rewritten

    title = _rewrite(title)
    caption = _rewrite(caption)

    if title.lower() == "small opening section":
        title = "Opening section does not stand out"
    if caption.lower() == "opening section area is too limited and visually not doing enough to guide visitors.":
        caption = (
            f"The top of the page does not do enough to show {audience_phrase} what matters most or where to look first."
        )

    if title.lower() == "opening section":
        title = "Opening section lacks emphasis"
    if title.lower() == "opening section crowding":
        title = "Opening section feels crowded"

    if "opening section" in caption.lower() and "look first" not in caption.lower():
...
def _render_annotations(annotations: list[dict[str, Any]], report_context: dict[str, Any]) -> str:
    if not annotations:
        return ""
    overlay_items = []
    caption_items = []
    for index, annotation in enumerate(annotations, start=1):
        ...
        title, caption = _plain_language_annotation(annotation, report_context)
        caption_items.append(
            f"""
            <li>
              <strong>{index}. {escape(title)}</strong><br />
              {escape(caption)}
            </li>
            """
        )
    ...
    *,
    ...
    if screenshots:
        for shot in screenshots:
            annotation_html = _render_annotations(shot.get("annotations", []), report_context)
            note_html = ""
